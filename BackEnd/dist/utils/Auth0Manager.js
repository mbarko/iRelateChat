...
module.exports = function () {
  ...
  function init() {
    return getToken().then(data => data.access_token).then(token => {
      const managementClientdata = new ManagementClient({
        domain: `${process.env.CLIENT_DOMAIN}`,
        token,
        audience: `https://${process.env.CLIENT_DOMAIN}/api/v2/`
      }); // set it so we can use it in our other methods

      managementClient = managementClientdata;
      return true;
    }).catch(err => err);
  }
  /**
  ...
  function getToken() {
    ...
    const url = `https://${process.env.CLIENT_DOMAIN}/oauth/token`; // make the call to the API via POST
  ...
  /**
   * Make a call to the Management API to get all the data for all users
   * All the things that are available in the dashboard can be accessed here
   * @param string clientId
   */


  function getUsers(clientId = null) {
    if (!clientId) clientId = process.env.CLIENT_ID;
    return managementClient.getUsers().then(Users => Users).catch(err => err);
  }
  /**
   * Make a call to the Management API to get all the data for spacific user
   * All the things that are available in the dashboard can be accessed here
   * @param string UserId
   * @param string clientId
   */
  ...
  function getUser(UserId, clientId = null) {
    if (!clientId) clientId = process.env.CLIENT_ID;
    return managementClient.getUser(UserId).then(User => User).catch(err => console.log(err));
  }
  /**
  ...
  function updateClient(data, clientId = null) {
    if (!clientId) clientId = process.env.CLIENT_ID;
    return managementClient.updateClient({
      client_id: clientId
    }, data).then(client => client).catch(err => err);
  }
}();